    ...
    for _ in 0..q {
        ...
        match t {
            1 => {
                ...
                s.push_back(x);
                ans = ans * ModInt::new(10) + ModInt::new(x);
            }
            2 => {
                let p = s.pop_front().unwrap();
                ans = ans - ModInt::new(p) * ModInt::new(10).pow(s.len());
            }
            3 => {
                println!("{}", ans.value());
            }
...
// ---------
// [Rustで競技プログラミングよくばりセット - Qiita](https://qiita.com/SakiKuroe/items/152a3c117a68d95faa0b#mod-int-mint-modint)
// ----------
use std::ops;
